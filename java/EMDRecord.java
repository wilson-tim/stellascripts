...
public class StellaAIRLoad {
    ...
    public static int processFile(File fileToProcess, boolean showContents,
        ...
        Vector EMDRecs = new Vector();
        Vector EMDTkts = new Vector();
                ...
                while ((oneLine = fileIn3.readLine()) != null) {
                    ...
                    if ((lineLength > 2) &&
                        ...
                        if (!voidTkt) {
                             ...
                             */
                                ...
                                // is actual fare changes
                                    ...
                                    // will be same as collection amt at FPO
                                    // record
                                    if ((Double.parseDouble(recSellingFare)) != (Double
                                            .parseDouble(collectionAmt))) {
                                        recSellingFare = "0";
                                    }
                                } else if ((oneLine.substring(0, 3)
                                ...
                                if (!exchangeFound) {
                                    ...
                                    if (!UnpaidTaxFound) { // Go through next
                                        ...
                                        if (Double
                                            .parseDouble(recPublishedFare) > 0) {
                                            recPublishedFare = Double
                                                .toString(Double
                                                    .parseDouble(recPublishedFare) -
                                                    totalAdditionalTax);
                                        }

                                        if (Double.parseDouble(recSellingFare) > 0) {
                                            recSellingFare = Double
                                                .toString(Double
                                                    .parseDouble(recSellingFare) -
                                                    totalAdditionalTax);
                                        }
                                    } // UnpaidTax found
                        ...
                        if ((recID.equalsIgnoreCase("EM")) &&
                            ...
                            if (!EMDfound) {
                                EMDfound = true;
                                application.log.finest("EMDfound ");
                            }
                            ...
                            if ((startPos > 0) && (endPos > 0) && (endPos > startPos)) {
                                ...
                                application.log.finest("EMD record created " + EMDRec.docID + " " + EMDRec.sellingFareAmount + " " + EMDRec.remainingTax );
                            }
                        }
                    } // not within the list
                } // end of while
            ...
            for (Enumeration recData = fileRecs.elements(); recData
                ...
                while (recData.hasMoreElements() &&
                    ...
                    if (fileRec.recordID.equals("T-")) {
                        ...
                        if (dashPos > 0) {
                            if ((fileRec.recordText.substring(dashPos + 1,
                                    dashPos + 3)).equalsIgnoreCase("00")) {
                                maxConjTktNum = Long.toString(Long
                                    .parseLong(recTicketNo) + 1);
                            } else {
                                maxConjTktNum = recTicketNo.substring(0, 8) +
                                    fileRec.recordText.substring(
                                        dashPos + 1, dashPos + 3);
                            }
                        } else {
                    ...
                    // JR added length > 10 to ignore processing for record like eg : FPNONREF, FPNONREFAGT
                        ...
                        if (fileRec.recordID.equals("FP") && (StringUtils.searchStringOccur(fileRec.recordText, "NONREFAGT", 1) > 0)) {
                            ...
                            if (rmrefs.length() > 2) { // Avoid processing erroneous data such as RM ##DRR
                                if (group.equals("")) {
                        ...
                        application.log.finest("inside for TMCD");
                        ...
                        dashPos = StringUtils.searchStringOccur(
                            tktRecord, "-", 1);
                        recAirline = tktRecord.substring(0, dashPos);
                        recTicketNo = tktRecord.substring(dashPos + 1, endPos);
                        
                        // Update associated EMD record in EMDTkts
                        // first get the document id
                        startPos = StringUtils.searchStringOccur(
                            tktRecord, ";", 2);
                        tktDocID = tktRecord.substring(startPos + 1);
                        ...
                        for (Enumeration EMDData = EMDRecs.elements(); EMDData
                            ...
                            if (EMDCurrentRec.docID.equals(tktDocID)) {
                                idxEMDFound = true;
                                application.log.finest("EMD and ticket docID is " + tktDocID);
                                application.log.finest("Ticket airline is " + recAirline);
                                application.log.finest("Ticket number is " + recTicketNo);
                                EMDCurrentRec.airline = recAirline;
                                EMDCurrentRec.ticketNo = recTicketNo;
                                break;
                            }
                            idxEMDRecs ++;
                        }
                        ...
                        application.log.finest("EMD record updated " + EMDCurrentRec.docID + " " + EMDCurrentRec.sellingFareAmount + " " + EMDCurrentRec.remainingTax + " " + EMDCurrentRec.airline + " " + EMDCurrentRec.ticketNo );
                    }

                    else if ((fileRec.recordID.equals("MF")) && fileRec.recordText.startsWith("MFPNONREFAGT")) {
                        stage = "processing MFP record ";
                        application.log.finest("inside for MFP");
                        application.log.finest("MFP line : " + fileRec.recordText);
                        
                        mfpRecord = fileRec.recordText.substring(12);

                        // Extract group and booking reference
                        endPos = StringUtils.searchStringOccur(
                            mfpRecord, ";", 1);
                        group = mfpRecord.substring(0, 1);
                        recBookingRef = mfpRecord.substring(1, endPos);
                        ...
                        application.log.finest("EMD and ticket docID is " + tktDocID);
                        application.log.finest("Ticket group is " + group);
                        application.log.finest("Ticket booking ref is " + recBookingRef);
                        ...
                        for (Enumeration EMDData = EMDRecs.elements(); EMDData
                            ...
                            if (EMDCurrentRec.docID.equals(tktDocID)) {
                                ...
                                application.log.finest("Ticket group is " + group);
                                application.log.finest("Ticket booking ref is " + recBookingRef);
                                EMDCurrentRec.group = group;
                                EMDCurrentRec.bookingRef = recBookingRef;
                                break;
                            }
                            idxEMDRecs ++;
                        }
                        ...
                        if (idxEMDFound) {
                            EMDRecs.set(idxEMDRecs, EMDCurrentRec);
                            application.log.finest("EMD record updated " + EMDCurrentRec.docID + " " + EMDCurrentRec.sellingFareAmount + " " + EMDCurrentRec.remainingTax + " " + EMDCurrentRec.airline + " " + EMDCurrentRec.ticketNo + " " + EMDCurrentRec.group + " " + EMDCurrentRec.bookingRef );
                        }
                        ...
                        if (idxEMDFound) {
                            ...
                            application.log.finest("EMD ticket record updated " + EMDCurrentRec.docID + " " + EMDCurrentRec.sellingFareAmount + " " + EMDCurrentRec.remainingTax + " " + EMDCurrentRec.airline + " " + EMDCurrentRec.ticketNo + " " + EMDCurrentRec.group + " " + EMDCurrentRec.bookingRef );
                        }
                    }
                    fileRec = (AirRecord) recData.nextElement();
