...
error_mail_list="${appdir}/a.lst"
user_mail_list="${appdir}/a.lst"
...
if [[ "${step_no}" -ge "${START_STEP_NO}" ]] then
  ...
  then
    ...
    do
      echo ${users}
      mailx -v -s "TESTING STELLA - `hostname` Duplicate Files Exist" ${users} < ${mailmessage}
    done
...
if [[ "${step_no}" -ge "${START_STEP_NO}" ]] then
  ...
  sqlplus -s ${dbuser}/${dbpass} @$appdir/run_air_load.sql > $appdir/run_air_load.lst
...
if [[ "${step_no}" -ge "${START_STEP_NO}" ]] then
  ...
  then
    ...
    do
      echo ${users}
      mailx -v -s "TESTING Details of STELLA AIR  errors encountered `hostname`  on "$today_d" " ${users} < ${appdir}/air_error.err
    done
...
if [[ "${step_no}" -ge "${START_STEP_NO}" ]] then
  ...
  do
    echo ${users}
    mailx -v -s "TESTING STELLA AIR load `hostname` on "$today_d" " ${users} < ${appdir}/air_mail.log
  done
else
  report "Step ${step_no} bypassed.\n"
fi
...
if [[ "${step_no}" -ge "${START_STEP_NO}" ]] then
  ...
  for arcfiles in `find ${stellabakdir} -name "[0-9]*.AIR" -mtime +0 -mtime -1`
  do 
